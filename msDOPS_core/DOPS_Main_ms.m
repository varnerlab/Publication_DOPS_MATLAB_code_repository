function [ g_best_solution,bestparticle,particle,fitness, bestval_dds_swarm, best_particle_dds_swarm,best_particles_ls,DDS_iters] =DOPS_Main_ms( optFunction,MAXJ,MINJ,NP,NI,NS,G,r)
...
while((num_iters_remaining > 0) && size(g_best_solution,2)<NI)
    ...
    if(count ==0)
        [g_best_solution_r,bestparticle_r,particle_r,fitness_r,num_iters_remaining] = DOPS_PSO_ms(optFunction,MAXJ,MINJ,NP,num_iters_remaining,NS,G,r);
    else
        if(num_iters_remaining == 1)
            %so that PSO will actually run
            num_iters_remaining = 2;
        end
        [g_best_solution_r,bestparticle_r,particle_r,fitness_r,num_iters_remaining] = DOPS_PSO_ms(optFunction,MAXJ,MINJ,NP,num_iters_remaining,NS,G,r,IC);
    end
    ...
    [bestval_dds_swarm_r,best_particle_dds_swarm_r,num_iters_remaining]=DOPS_DDS_ms(optFunction,IC,MAXJ,MINJ,r,num_iters_remaining);
    DDS_iters = cat(2,DDS_iters,num_iters_remaining);
    %concat results
